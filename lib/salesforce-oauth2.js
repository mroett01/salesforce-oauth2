...
var oauth2 = module.exports = function (options, callback) {
    ...
    if (options.revoke)
    {
        options = _.omit(options, "revoke");
        uri = base_url + revokeUrl + "?" + qs.stringify(options);
    }
    else
        uri = base_url + tokenUrl + "?" + qs.stringify(options);
...
oauth2.revoke = function (options, callback) {
    options = _.extend({
        "revoke" : "true"
    }, options);
    return oauth2(options, callback);
